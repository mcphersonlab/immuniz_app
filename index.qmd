      ...
      <div class="card-body">
        ...
        <div class="row g-2">
          ...
          <div class="col-6">
            ...
            <div class="form-check">
              <input class="form-check-input contra" type="checkbox" id="c-fear-of-needles" value="fear-of-needles" />
              <label class="form-check-label small" for="c-fear-of-needles">Fear of needles (patient prefers intranasal route)</label>
            </div>
...
const SCHEDULE = [
  ...
  // Influenza (IIV/ccIIV)
  {
    ...
    catchupMinDays: toDays(6, 'months'),
    catchupMaxDays: toDays(120, 'years'),
    notes: "Recommended annually for all persons ≥ 6 months. Two doses (4 weeks apart) in first season if < 9 years and not previously vaccinated. Severe egg allergy: use ccIIV4 or RIV4.",
    contraindications: ['prev-reaction'],
  ...
  },
  {
    ...
    abbr: "LAIV4",
    series: "Annual (alternative when IIV4 contraindicated or needle-averse)",
    minAgeDays: toDays(2, 'years'),
    maxAgeDays: toDays(49, 'years'),
    catchupMinDays: toDays(2, 'years'),
    catchupMaxDays: toDays(49, 'years'),
    notes: "Alternative (intranasal) option shown when IIV4 is contraindicated or patient has fear of needles. For healthy, non-pregnant persons 2–49 years. Avoid in: first-time vaccination (< 9 yrs), egg allergy, asthma or recurrent wheezing (especially < 5 yrs), and immunocompromising conditions.",
...
const CONTRA_LABELS = {
  ...
  'fear-of-needles':   'Patient preference for intranasal route'
};
...
function computeSchedule(ageDays, contraKeys) {
  ...
  // Influenza dedup: show IIV4 by default; show LAIV4 only if IIV4 is
  // contraindicated (absent from recommended) or patient has fear of needles.
  const hasIIV4  = recommended.some(r => r.abbr === 'IIV4');
  const hasLAIV4 = recommended.some(r => r.abbr === 'LAIV4');
  if (hasIIV4 && hasLAIV4) {
    if (contraSet.has('fear-of-needles')) {
      // Patient prefers intranasal: suppress IIV4, show LAIV4
      const iivIdx = recommended.findIndex(r => r.abbr === 'IIV4');
      if (iivIdx !== -1) recommended.splice(iivIdx, 1);
    } else {
      // Default: suppress LAIV4, show IIV4
      const laivIdx = recommended.findIndex(r => r.abbr === 'LAIV4');
      if (laivIdx !== -1) recommended.splice(laivIdx, 1);
    }
  }
