...
function renderResults(ageDays) {
  ...
  // Separate received vaccines from recommended/catchup
  const received = [];
  const filteredRecommended = recommended.filter(row => {
    if (receivedSet.has(`${row.abbr}|||${row.series}`)) { received.push(row); return false; }
    return true;
  });
...
// Debounced version of autoUpdate for age-value input
const debouncedAutoUpdateForAge = debounce(autoUpdate, 400);

// ── Event Listeners ────────────────────────────────────────────────
document.getElementById('dob').addEventListener('input', autoUpdate);
document.getElementById('age-value').addEventListener('input', debouncedAutoUpdateForAge);
document.getElementById('age-unit').addEventListener('change', autoUpdate);
